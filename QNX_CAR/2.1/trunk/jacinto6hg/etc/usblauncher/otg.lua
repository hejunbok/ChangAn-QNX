...
Host_Stack = {
        cmd  = 'io-usb -cvvv -d omap5-xhci ioport=0x488d0000,irq=110';
        path = '/dev/otg/io-usb';
}
...
Device_Stack = {
        cmd  = 'io-usb-dcd -d usbncm-omap543x-dwcotg3 ioport=0x488d0000,irq=110,verbose=0';
--      cmd  = 'io-usb-dcd -vvvvvv -d iap2ncm-omap543x-dwcotg3 ioport=0x4a030000,irq=124,verbose=6';
        path = '/dev/otg/io-usb-dcd';
--      descriptors = { iap2, iap2ncm, umass };
        descriptors = { iap2, iap2ncm };
        timeout = 6000;
}
...
device(0x1234, 4) {
        class(USB_CLASS_COMMS, USB_COMMS_SUBCLASS_MODEM, 0) {
                driver"devc-serusb_dcd -e -v -F -s -d iface_list=0,unit=1,path=/dev/otg/io-usb-dcd"
        },
        class(USB_CLASS_MASS_STORAGE)
        {
                driver"devu-umass_client-block -s /dev/otg/io-usb-dcd  -l lun=0,devno=1,iface=0,fname=/dev/ram0"
        }
}
