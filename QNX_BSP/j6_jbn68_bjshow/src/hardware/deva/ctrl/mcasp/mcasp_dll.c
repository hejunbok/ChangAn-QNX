...
int sample_rate_divides_ahclk(struct ahclk *ahclk, unsigned rate)
{
	...
	for (i = 0; i < ahclk->num_freqs; i++)
	{
		...
		//ado_debug(DB_LVL_DRIVER, "### rate %u freq %u", __func__, rate, freq);
					  ...
					  ado_pcm_config_t * config, ado_pcm_subchn_t * subchn, uint32_t * why_failed)
{
	 ...
	 *             the FIFO threshold to its maximum (Largest multiple of active serializers)
	 *             The FIFO size is 64, so limit to 32 words to prevent overrun/underrun.
	 */
	fifo_thres = 32 / mcasp_card->play_strm.serializer_cnt * mcasp_card->play_strm.serializer_cnt;
	...
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].BCNT = fifo_thres;
	...
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].SRCCIDX = mcasp_card->sample_size * fifo_thres;
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].DSTCIDX = 0;
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].CCNT =
		ado_pcm_dma_int_size(config) / (mcasp_card->sample_size * fifo_thres);	/* Number of frame/time_slot per param set */
...
mcasp_capture_aquire(HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T ** pc,
					 ado_pcm_config_t * config, ado_pcm_subchn_t * subchn, uint32_t * why_failed)
{
	int idx, opened_subchns = 0, fifo_thres = 0;
	...
	if(0 == opened_subchns)
	{
		 ...
		 *             The FIFO size is 64, so limit to 32 words to prevent overrun/underrun.
		 */
		fifo_thres = 32 / mcasp_card->cap_aif.serializer_cnt * mcasp_card->cap_aif.serializer_cnt;
		...
		mcasp_card->edma3->PaRAM[strm->dma_idx].BCNT = fifo_thres;
		...
		mcasp_card->edma3->PaRAM[strm->dma_idx].DSTCIDX = mcasp_card->sample_size * fifo_thres;
		mcasp_card->edma3->PaRAM[strm->dma_idx].CCNT =
			mcasp_card->cap_aif.frag_size / (mcasp_card->sample_size * fifo_thres);    /* Number of frame/time_slot per param set */
		/* Reload Param set */
...
int32_t mcasp_prepare(HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T * subchn, ado_pcm_config_t * config)
{
	...
	if (subchn->strm == &mcasp_card->play_strm)
	{
		...
		mcasp_card->edma3->PaRAM[strm->dma_idx].CCNT =
			(ado_pcm_dma_int_size(config) / (mcasp_card->sample_size * fifo_thres));
	}
	else
	{
		...
		if(mcasp_card->cap_aif.active == 0)
		{
			...
			/* Reset the CCNT (Frames per param set) */
			mcasp_card->edma3->PaRAM[strm->dma_idx].CCNT = 
				(mcasp_card->cap_aif.frag_size / (mcasp_card->sample_size * fifo_thres));
		}
		ado_mutex_unlock(&mcasp_card->hw_lock);
...
int32_t mcasp_trigger(HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T * pc, uint32_t cmd)
{
	...
	if (cmd == ADO_PCM_TRIGGER_GO)
	{
		...
		if (pc == &mcasp_card->play_strm.subchn)
		{
			 ...
			 *             The FIFO size is 64, so limit to 32 words to prevent overrun/underrun.
			 */
			int fifo_thres = 32 / mcasp_card->play_strm.serializer_cnt * mcasp_card->play_strm.serializer_cnt;

			mcasp_card->mcasp->WFIFOCTL = WFIFOCTL_WNUMEVT(fifo_thres) | WFIFOCTL_WNUMDMA(mcasp_card->play_strm.serializer_cnt);
			...
			timeout = 1000;
			while (((mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL) < 63) && timeout--)
				nanospin_ns(100);
			if (timeout <= 0)
		...
		else
		{
			if (mcasp_card->cap_aif.active == 0)
			{
				 ...
				 *             The FIFO size is 64, so limit to 32 words to prevent overrun/underrun.
				 */
				int fifo_thres = 32 / mcasp_card->cap_aif.serializer_cnt * mcasp_card->cap_aif.serializer_cnt;

				mcasp_card->mcasp->RFIFOCTL = RFIFOCTL_RNUMEVT(fifo_thres) | RFIFOCTL_RNUMDMA(mcasp_card->cap_aif.serializer_cnt);
...
{
	int32_t nbytes = ado_pcm_dma_int_size(mcasp_card->play_strm.subchn.pcm_config) -
					(mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].CCNT * mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].SRCCIDX);

#if !defined(VARIANT_j2_pcm3168a)
	ado_error("mcasp_card: TX Underrun occured! - TX AFIFO count = %d, DMA position = %d/%d,", mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL,
				nbytes, ado_pcm_dma_int_size(mcasp_card->play_strm.subchn.pcm_config));
#else
	ado_error("mcasp_card: TX Underrun occured! - DMA Position = %d/%d", nbytes, ado_pcm_dma_int_size(mcasp_card->play_strm.subchn.pcm_config));
#endif
	mcasp_card->mcasp->XSTAT |= XSTAT_XUNDRN;
}
	...
	while (args != NULL && args[0] != 0)
	{
		switch ((opt = getsubopt(&args, opts, &value)))
		{
			...
			case 29:
				...
				break;
			default:
				ado_error("%s: Invalid argument %s", __FUNCTION__, args);
				break;
		}
	}
...
unsigned find_supported_rates(struct ahclk *ahclk, unsigned rate_min, unsigned rate_max)
{
	...
	for (i = 0; i < ARRAY_SIZE(supported_rate_list); i++)
	{
		...
		ado_debug(DB_LVL_DRIVER, "### rate %u rate_min %u rate_max %u", __func__, rate, rate_min, rate_max);
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/deva/ctrl/mcasp/mcasp_dll.c $ $Rev: 766663 $")
#endif
