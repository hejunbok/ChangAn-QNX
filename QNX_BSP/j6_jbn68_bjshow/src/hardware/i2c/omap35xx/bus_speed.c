...
omap_set_bus_speed(void *hdl, unsigned int speed, unsigned int *ospeed)
{
    ...
    //only for ipod_auth ic
    if ( dev->physbase == OMAP5_I2C5_PHYSBASE && dev->slave_addr == IPOD_AUTH_IC_SLAVE_ADDRESS )
        speed=50000;

    if (dev->options & OMAP_OPT_VERBOSE) {
        slogf(_SLOGC_I2C, _SLOG_INFO, "i2c-omap35xx (%s): speed:%u", __func__, speed);
    }
