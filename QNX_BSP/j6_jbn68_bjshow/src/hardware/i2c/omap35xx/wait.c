				...
				if (dev->xlen) {
					*dev->buf++ = (uint8_t)(in16(dev->regbase + OMAP_I2C_DATA) & 0x00FF);
					...
					if (dev->options & OMAP_OPT_VERBOSE) {
						slogf(_SLOGC_I2C, _SLOG_INFO,"i2c-omap35xx (%s) - recv irq: buf: %02x", __func__, *dev->buf);
						}
#endif
					dev->xlen--;
				} else {
...
#if 0					//log for debug
					if (dev->options & OMAP_OPT_VERBOSE) {
						slogf(_SLOGC_I2C, _SLOG_INFO,"i2c-omap35xx (%s) - send irq: buf: %02x", __func__, *dev->buf);
						}
#endif
