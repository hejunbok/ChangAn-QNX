...
void put_scif(int c)
{
	...
	while( !(in16(base + SH_SCIF_SCFSR_OFF) & SH_SCIF_SCFSR_TDFE) );
	out8(base + SH_SCIF_SCFTDR_OFF, c);
	out16(base + SH_SCIF_SCFSR_OFF, 0);
	while( !(in16(base + SH_SCIF_SCFSR_OFF) & SH_SCIF_SCFSR_TEND) );
}
