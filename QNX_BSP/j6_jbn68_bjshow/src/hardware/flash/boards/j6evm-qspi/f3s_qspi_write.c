                         ...
                         uint8_t *buffer)
{
    ...
    while (left) {
        ...
        // write data to flash
        rc = qspi_flash_page_program(qspi, off, cursz, buffer);
        if (rc != EOK) {
            errno = EIO;
            return -1;
        }
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/flash/boards/j6evm-qspi/f3s_qspi_write.c $ $Rev: 738509 $")
#endif
