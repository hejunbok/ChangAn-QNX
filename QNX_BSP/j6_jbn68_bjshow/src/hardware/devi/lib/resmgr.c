...
resmgr_create_device (event_bus_line_t * line)
{
	...
	memset (attr, 0, sizeof (devi_attr_t));
	...
	iofunc_attr_init (&attr->attr, 0660 | S_IFCHR, NULL, NULL);
...
devi_devctl (resmgr_context_t * ctp, io_devctl_t * msg, RESMGR_OCB_T * ocb)
{
	...
	switch (msg->i.dcmd) {
	/* NOTE Erick: Remove this later, QNX Car guys need some way to calibrate until screen calib stuff is done */
	case DCMD_SET_PTR_CTRL:
	{
		...
		if (verbosity) 
			fprintf (stderr, "DCMD_SET_PTR_CTRL(%s), module=%p\n", *(int *)dptr & 0x01 ? "cooked" : "raw", module);

		abs_mode.mode = *(int *)dptr & 0x01 ? ABS_XLATE_MODE : ABS_RAW_MODE;
	...
	default:
	{ /* Not sure if anyone uses this still, keep it as the default action just in case */
		attr = ocb->ocb.attr;
		error = EOK;
		...
		if (!(get_device_direction (dcmd = msg->i.dcmd) & DEVDIR_TOFROM))
			...
			memset (buffer, 0, sizeof ((nbytes + 3) & ~3));
			memset (allocd, 0, sizeof ((nbytes + 3) & ~3));
			SETIOV (&iov, buffer, nbytes);
			if (resmgr_msgreadv (ctp, &iov, 1, sizeof (msg->i)) == -1)
				goto error_out;
		}

		/* NOTE: now call a helper function to actually do the devctl
...
add_ocb (devi_attr_t * attr, RESMGR_OCB_T * ocb)
{
	...
	if (attr->ocb_list == NULL) {
		...
		memset (attr->ocb_list, 0, sizeof (struct _ocb_list));
		...
		memset (op->next, 0, sizeof (struct _ocb_list));
		op->next->ocb = ocb;
		op->next->prev = op;
		op->next->next = NULL;
	}
								...
								int nbytes)
{
	...
	memset (wp, 0, sizeof (struct _wait_q));
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devi/lib/resmgr.c $ $Rev: 736689 $")
#endif
