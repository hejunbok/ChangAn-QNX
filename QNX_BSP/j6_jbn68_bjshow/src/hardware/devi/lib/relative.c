...
rel_input (input_module_t * module, int num, void *arg)
{
	...
	for (; num; num--) {
		...
		if (verbosity >= 3)
			printf ("Raw Mouse Data X: %d Y: %d Z: %d Buttons: %d\n", p->dx, p->dy, p->dz, p->buttons);
		...
		if (dp->flags & FLAG_RWHEEL) {
			...
			if ((dp->zmode != ZMODE_SEND_CLIENT_ONLY) &&
					(NULL != pkbd) && (NULL != pkbd->data) && (!(OptFlags & OPT_NO_PHOTON))) {
				int zm = zmove;
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devi/lib/relative.c $ $Rev: 680332 $")
#endif
