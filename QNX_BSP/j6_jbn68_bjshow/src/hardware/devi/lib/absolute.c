...
#define DIFFERENTIAL_FILTER       0x0040
#define AVERAGING_FILTER          0x0080
...
#define FLAG_PACKET_COMPRESSION   0x8000
...
struct data
{
	...
	unsigned lastx, lasty, lastz, lastb;
	timer_t         timerid;
	...
	/* Packet Compression */
	int compression_ratio;  /* Number of packets to compress into 1 */
	...
	/* Filter Values */
	int filter_data_count;
	unsigned *unfiltered_x;
	unsigned *unfiltered_y;

	int averaging_filter_points;
...
static int filter (struct data *dp, struct packet_abs *filtered_point);
...
static int abs_input (input_module_t * module, int num, void *ptr)
{
	...
	for (i = 0; i < num; i++, abuf++) { 
		...
		if (dp->translate) {
			...
			/* Call the filter code, if we asked for it and the current event is not a release event */
			if ((dp->filter_type) && (tmp.buttons)) {
				if (filter (dp, &tmp) == -1) {
					continue;
				}
			} else {
...
static int abs_parm (input_module_t * module, int opt, char *optarg)
{
	...
	case 'D':
		dp->filter_type = DIFFERENTIAL_FILTER;
		dp->max_delta = atoi (optarg); 
		dp->unfiltered_x = malloc (3);
		dp->unfiltered_y = malloc (3);
		break;
	case 'A':
		dp->filter_type = AVERAGING_FILTER;
		dp->averaging_filter_points = atoi (optarg);
		dp->unfiltered_x = malloc (dp->averaging_filter_points);
		dp->unfiltered_y = malloc (dp->averaging_filter_points);
		break;
	case 'N':
		dp->filter_type = NOISE_FILTER;
		dp->max_delta = atoi (optarg);
		dp->unfiltered_x = malloc (3);
		dp->unfiltered_y = malloc (3);
		break;
	case 'S':
		...
		break;
	case 'C':
		dp->compression_ratio = atoi (optarg); 
		dp->flags |= FLAG_PACKET_COMPRESSION;
		break;
	case 'o':
		p = optarg;
		...
		if (!*p) {
			const static char *emsg = "incorrect argument for absolute device '-o' option";
			fprintf (stderr, "%s\n", emsg);
			slogf (_SLOG_SETCODE (_SLOGC_INPUT, 0), _SLOG_ERROR, emsg);
			exit (EXIT_FAILURE);
		}
	...
	case 's':
		...
		if (!*p) {
			const static char *emsg = "incorrect argument for absolute device '-s' option";
			fprintf (stderr, "%s\n", emsg);
			slogf (_SLOG_SETCODE (_SLOGC_INPUT, 0), _SLOG_ERROR, emsg);
			exit (EXIT_FAILURE);
		}
...
/*=============================================================================
  ...
  Description:  Simple filter that just gets an average of the data points.

=============================================================================*/

unsigned averaging_filter (struct data *dp, unsigned dirty_position[]) 
{
	int i;
	unsigned val = 0;

	for (i = 0; i < dp->averaging_filter_points; i++)
		val += dirty_position[i];

	return (val/dp->averaging_filter_points);
...
/*=============================================================================
  ...
  Name:         differential_filter 

  Description:  Simple filter to determine if the delta between the last emited
                point and the current point is large enough to issue another
                event.  This is helpful to reduce the amount of data points
                comming in from the controller.

=============================================================================*/

int differential_filter (struct data *dp, struct packet_abs *current_position)
{
	static unsigned last_emited_x = -1, last_emited_y = -1;

	if ((abs (current_position->x - last_emited_x) < dp->max_delta) && (abs (current_position->y - last_emited_y) < dp->max_delta))
		return (-1);

	last_emited_x = current_position->x;
	last_emited_y = current_position->y;
...
/*=============================================================================
  ...
  Description:  Filter that tries to eliminate jittery noise from the controller.
                This should be used with SPI or I2C based touchscreens.
...
unsigned noise_filter (struct data *dp, unsigned dirty_position[])
{
	...
	if ((diff[0] > dp->max_delta) || (diff[1] > dp->max_delta) || (diff[2] > dp->max_delta)) {
		slogf (_SLOG_SETCODE (_SLOGC_INPUT, 0), _SLOG_ERROR, "Absolute Noise Filter Rejected Packet: Data is invalid, max noise exceeded.");

		if (verbosity > 2)
			fprintf (stderr, "Absolute Noise Filter Rejected Packet: Data is invalid, max noise exceeded.\n");
...
/*=============================================================================
  ...
  Description:  This function simply applies the filters specified to the abs
                module.

=============================================================================*/

int filter (struct data *dp, struct packet_abs *packet)
{
	/* Fill the array the array is used for the noise and average filters */
	dp->unfiltered_x[dp->filter_data_count] = packet->x;
	dp->unfiltered_y[dp->filter_data_count] = packet->y;
	dp->filter_data_count++;
	...
	case BUTTON_FILTER:
		...
		break;
	case NOISE_FILTER:
		if (dp->filter_data_count > 3) {
			packet->x = noise_filter (dp, dp->unfiltered_x);
			packet->y = noise_filter (dp, dp->unfiltered_y);
			dp->filter_data_count = 0;
		}	else {
			return (-1);
		}
		break;
	case DIFFERENTIAL_FILTER:
		dp->filter_data_count = 0;

		if (differential_filter (dp, packet))
			return (-1);
		break;
	case AVERAGING_FILTER:
		if (dp->filter_data_count >= dp->averaging_filter_points) {
			packet->x = averaging_filter (dp, dp->unfiltered_x);
			packet->y = averaging_filter (dp, dp->unfiltered_y);
			dp->filter_data_count = 0;
		} else {
			return (-1);
		}
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devi/lib/absolute.c $ $Rev: 680332 $")
#endif
