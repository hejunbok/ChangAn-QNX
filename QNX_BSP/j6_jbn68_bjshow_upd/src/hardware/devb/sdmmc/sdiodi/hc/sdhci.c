...
//		hc->caps	|= HC_CAP_HS200;
	...
	if( ( cap & SDHCI_CAP_DMA ) ) {
		if( hc->version >= SDHCI_SPEC_VER_2 ) {
			...
			if( ( sdhc->adma = mmap( NULL, sizeof( sdhci_adma32_t ) * ADMA_DESC_MAX, PROT_READ | PROT_WRITE | PROT_NOCACHE, 
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devb/sdmmc/sdiodi/hc/sdhci.c $ $Rev: 747704 $")
#endif
