...
static const struct callout_slot callouts_hvc[] =
{
    { CALLOUT_SLOT( reboot, _hvc ) },
};
...
const struct debug_device debug_devices[] =
{
    [DEBUG_HVC] {
        .name = "hvc",
        .defaults[DEBUG_DEV_CONSOLE] = "",
        .init = init_hvc,
        .put = put_hvc,
        .callouts[DEBUG_DISPLAY_CHAR] = &display_char_hvc,
        .callouts[DEBUG_POLL_KEY] = &poll_key_hvc,
    },
    [DEBUG_8250] {
        .name = "8250",
        .defaults[DEBUG_DEV_CONSOLE] = "0x4806A000^2.115200.48000000.16",   // UART 1, use the baud rate set by boot loader
        .init = init_omap,
        .put = put_omap,
...
int main(int argc, char **argv, char **envv)
{
    ...
    if(in_hvc) {
        ...
        select_debug(&debug_devices[DEBUG_8250], sizeof(debug_devices[DEBUG_8250]));
    }
    ...
    if(in_hvc) {
        add_typed_string(_CS_MACHINE, "TI-DRA74X-EVM-HYP");
    } else {
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/startup/boards/dra74x/vayu-evm/main.c $ $Rev: 771583 $")
#endif
