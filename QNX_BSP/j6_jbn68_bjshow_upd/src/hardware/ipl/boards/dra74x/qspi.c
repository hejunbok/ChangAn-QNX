...
int qspi_s25fl_ident()
{
	...
	//ser_putstr("qspi_s25fl_ident:");
	//ser_puthex(buf[0]);
	//ser_puthex(buf[1]);
	//ser_puthex(buf[2]);

	if (buf[0] == MID_SPANSION) {
		if ((buf[1] == 0x02) && (buf[2] == 0x15)) {
			qspi.size = 32 * 1024 *1024;
			log_putstr("S25FL032P detected.\n");
...
static int qspi_n25q_ident()
{
	...
	if (buf[0] == MID_MICRON) {
		if ((buf[1] == (DID_N25Q128 & 0xFF)) && (buf[2] == (DID_N25Q128 >> 8))) {
			qspi.size = 16 * 1024 *1024;
			log_putstr("N25Q128A detected.\n");
			return 0;
		}
		if ((buf[1] == (DID_N25Q64 & 0xFF)) && (buf[2] == (DID_N25Q64 >> 8))) {
			qspi.size = 8 * 1024 *1024;
			log_putstr("N25Q64A detected.\n");
			return 0;
		}
	}
...
#else	//s25fl032p
	/* send erase command */
	qspi_send_command(QSPI_CMD_3SE_032p, 1, COMMAND_1_BYTE);

	/* issue 4 bytes sector address */
	return qspi_send_command(offset, 2, COMMAND_3_BYTE);
#endif
}
...
#else  //for s25fl032p
	// issue page program command plus 3 bytes address
	qspi_send_command(QSPI_CMD_3PP_032p, size + 1, COMMAND_1_BYTE);
	qspi_send_command(offset, size + 1, COMMAND_3_BYTE);

	// write data
	while (size--) {
		qspi_send_command( *buf++, size, COMMAND_1_BYTE);
	}
#endif
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/ipl/boards/dra74x/qspi.c $ $Rev: 772559 $")
#endif
