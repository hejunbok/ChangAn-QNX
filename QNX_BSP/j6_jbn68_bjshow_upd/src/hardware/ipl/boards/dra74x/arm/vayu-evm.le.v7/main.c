...
IPL_BootOpt_t ipl_boot_menu()
{
	...
	while (1) {
		...
		ser_putstr("Press 'C' for CAR2 EMMC download, file QNX-CAR2 assumed.\n");
...
int main(void)
{
	...
	while (1) {
		...
		switch (bootOpt) {
			case IPL_BOOT_SD:
				omap_usec_delay(50000);;
				if (sdmmc_load_file(SD_INTERFACE, 1, image, "QNX-IFS", 0) != 0) {
					ser_putstr("load image from SD failed, try emmc boot\n");
			...
			case IPL_BOOT_EMMC:
				...
				break;
			case IPL_BOOT_CAR2:
				if (sdmmc_load_file(EMMC_INTERFACE, 2, image, "QNX-CAR2", 0) != 0) {
					ser_putstr("load image from eMMC Car2 failed\n");
					goto print_boot_menu;
				}
				break;
			case IPL_BOOT_QSPI:
			...
			case IPL_BOOT_SERIAL:
				...
				break;
			case IPL_MEM_TEST:
				sdram_test(DRA74x_EMIF_CS0_BASE, MEG(2032));
			default:
				goto print_boot_menu;
		}
