...
loop:
	...
	while (1 == fread (&rec, sizeof (rec), 1, playFile)) {
		...
		/* NOTE ERICK: Switch the recording method to allow for line data (if possible), as that is what resmgr cares about */	
		//resmgr_enqueue_packet (rec.line, buf, rec.size);
	...
	input_shutdown ();						// NOTE: Do we need to exit?
	return NULL;
}
...
begin (int argc, char *argv[])
{
	...
	if (user_parm != NULL) {
		...
		if(setgid(gid) == -1 ||	setuid(uid) == -1 ) {
			fprintf(stderr, "%s: unable to set uid/gid - %s", "devi-hid", strerror(errno));
			return -1;
		}
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/devi/lib/begin.c $ $Rev: 736689 $")
#endif
