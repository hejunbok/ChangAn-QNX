...
int omap4430_cfg(void *hdl, spi_cfg_t *cfg)
{
	...
	if (!(cfg->mode & SPI_MODE_CSPOL_HIGH))
		ctrl |= OMAP4430_MCSPI_CSPOL_ACTIVE_LOW;	/* Bit Order */ 
	
    if (!(cfg->mode & SPI_MODE_CKPOL_HIGH))
		ctrl |= OMAP4430_MCSPI_POLARIY;	/* Active low polarity */
		
	if (cfg->mode & SPI_MODE_CKPHASE_HALF)
		ctrl |= OMAP4430_MCSPI_PHASE;	/* CPHA 1 Phase */	
	...
	*/
    ctrl &= OMAP4430_MCSPI_IS_SOMI_INPUT;
    ctrl &= OMAP4430_MCSPI_DPE1_SIMO_OUTPUT;
    ctrl |= OMAP4430_MCSPI_DPE0_TX_DISABLE;

   if (dev->cs_delay)
   {	
      // Set the CS to start of data delay to 1 1/2 clocks
      ctrl &= OMAP4430_MCSPI_CS_DELAY_MASK;
      ctrl |= OMAP4430_MCSPI_CS_DELAY_ONE_AND_A_HALF;
	}
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/spi/omap4430/config.c $ $Rev: 716138 $")
#endif
