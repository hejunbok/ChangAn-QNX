 ...
 */
static int32_t
aic3106_mixer_audioman_refresh_set(MIXER_CONTEXT_T * hw_ctx, ado_dswitch_t * dswitch, snd_switch_t * cswitch,
								void *instance_data)
{
	return (EOK);
}
...
static int32_t
aic3106_mixer_audioman_refresh_get(MIXER_CONTEXT_T * hw_ctx, ado_dswitch_t * dswitch, snd_switch_t * cswitch,
								void *instance_data)
{
...
static int32_t build_aic3106_mixer(MIXER_CONTEXT_T * aic3106, ado_mixer_t * mixer)
{
	...
	if(SEL_MCASP2 == aic3106->sel_mcasp)
	{
		...
		// Audioman support
		if (!error && ado_mixer_switch_new(mixer, "Audioman Refresh", SND_SW_TYPE_BOOLEAN, 0, aic3106_mixer_audioman_refresh_get,
											aic3106_mixer_audioman_refresh_set, NULL, NULL) == NULL)
			error++;
	...
	for(j=0, i = aic3106->i2c_addr1; i <= (aic3106->i2c_addr1 + aic3106->num_of_codecs - 1); i++, j++)
	{
		...
		if( MASTER == aic3106->hwc->clk_mode)
		{
			...
			*/
			...
			aic3106_wr(aic3106, AIC3106_ADDITIONAL_GPIO_CTRL_B, CODEC_CLKIN_SOURCE_PLLDIV, i);
			aic3106_wr(aic3106, AIC3106_CLK_GENERATION_CTRL, (CLKDIV_IN_USE_BCLK | PLLCLK_USE_BCLK), i);
			aic3106_wr(aic3106, AIC3106_PLL_PROGRAM_REG_B, 32, i);  // J=32
			aic3106_wr(aic3106, AIC3106_PLL_PROGRAM_REG_C, 0 , i);  // 00: MSB of D
			aic3106_wr(aic3106, AIC3106_PLL_PROGRAM_REG_D, 0, i);   // 00: LSB of D
			aic3106_wr(aic3106, AIC3106_AUDIO_CODEC_OVERFLOW_FLAG, 1, i);   // R=1
			aic3106_wr(aic3106, AIC3106_PLL_PROGRAM_REG_A, (REG_A_PLL_CTRL | 1), i);  // Enable PLL, P = 1
		}
		else
		...
		else if(SEL_MCASP5 == aic3106->sel_mcasp)
		{
			...
			aic3106_wr(aic3106, AIC3106_LINE1L_TO_LEFT_ADC_CTRL, (1<<7) | (0<<2),  i);

			// LINE1R to Right ADC: fully differential & volume (0db), power-on
			aic3106_wr(aic3106, AIC3106_LINE1R_TO_RIGHT_ADC_CTRL, (1<<7) | (0<<2),  i);
		}
		...
		aic3106_wr(aic3106, AIC3106_DAC_OUTPUT_SWITCH_CTRL, (RIGHT_DAC_OUT_SWITCHING_CTRL | LEFT_DAC_OUT_SWITCHING_CTRL), i);
...
codec_mixer(ado_card_t * card, HW_CONTEXT_T * hwc)
{
	...
	if(hwc->mcasp_baseaddr == MCASP2_BASEADDR || hwc->mcasp_baseaddr == MCASP4_BASEADDR)
	...
	aic3106->tx_voices = hwc->play_strm.voices;
	aic3106->rx_voices = hwc->cap_aif.cap_strm[0].voices;
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/hardware/deva/ctrl/mcasp/nto/arm/dll.le.j6_aic3106.v7/aic3106.c $ $Rev: 759887 $")
#endif
