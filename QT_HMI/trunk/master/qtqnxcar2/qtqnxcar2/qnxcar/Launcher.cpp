...
void Launcher::Private::ppsLauncherReply(const QPps::Changeset &changes) {
    ...
    qDebug("%s: msg: '%s', res: '%s', id: '%s', dat: '%s', errorcode: '%s'", Q_FUNC_INFO,
           qPrintable(message.msg),
           qPrintable(message.res),
           qPrintable(message.id),
           qPrintable(message.dat),
           qPrintable(message.errorcode));
    ...
    if(message.msg == QStringLiteral("stopped")) {
        ...
        qDebug("%s: Application with PID %lld has stopped", Q_FUNC_INFO, pid);
        ...
        if(message.errorcode != NULL && !message.errorcode.isNull() && !message.errorcode.isEmpty()) {
           ...
           emit q->applicationStartFailed();
            ...
            qDebug("%s: Application with PID %lld has started", Q_FUNC_INFO, pid);
