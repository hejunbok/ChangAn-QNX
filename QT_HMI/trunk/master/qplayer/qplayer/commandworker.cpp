...
void CommandWorker::getMediaSources(MediaSourcesCommand *command)
{
    ...
    if (rc == NO_ERROR) {
        ...
        qDebug() << Q_FUNC_INFO << "Emitting command complete";
        ...
        qDebug() << Q_FUNC_INFO << "Freeing media source memory";
...
void CommandWorker::getPlayerState(PlayerStateCommand *command)
{
    ...
    if (rc == NO_ERROR) {
        ...
        qDebug() << Q_FUNC_INFO << "Freeing player state memory";
        free(playerState);
    } else {
...
void CommandWorker::getCurrentTrack(CurrentTrackCommand *command)
{
    ...
    if (rc == NO_ERROR) {
        ...
        qDebug() << Q_FUNC_INFO << "Freeing track info memory";
...
void CommandWorker::getExtendedMetadata(ExtendedMetadataCommand *command)
{
    ...
    qDebug() << Q_FUNC_INFO << "Getting extended metadata for node ID" << nodeIdStr;
...
void CommandWorker::browse(BrowseCommand *command)
{
    ...
    qDebug("%s mediaSourceId=%d, mediaNodeId='%s', offset=%d, limit=%d",
           Q_FUNC_INFO,
    ...
    while(!done && !error) {
        ...
        qDebug("%s Calling mm_player_browse with offset=%d, limit=%d", Q_FUNC_INFO, command->offset() + totalNodes, nodesThisChunk);
        ...
        qDebug("%s mm_player_browse: nodesThisChunk=%d, numRequested=%d, rc=%d", Q_FUNC_INFO, nodesThisChunk, numRequested, rc);

        if(rc == NO_ERROR) {
            if (nodesThisChunk == 0) {
                // EOF -- there are no more nodes -- totalNodes is a subset of numRequested
                qDebug("%s EOF reached: numRequested=%d, totalNodes=%d", Q_FUNC_INFO, numRequested, totalNodes);
                done = true;
            } else {
                if (nodeChunk != NULL)  {
                    ...
                    if (totalNodes == numRequested) {
                        ...
                        qDebug("%s Got all requested nodes: numRequested=%d, totalNodes=%d", Q_FUNC_INFO, numRequested, totalNodes);
                    }
...
void CommandWorker::search(SearchCommand *command)
{
    ...
    qDebug("%s mediaSourceId=%d, searchTerm='%s', filter='%s', offset=%d, limit=%d",
           Q_FUNC_INFO,
           command->mediaSourceId(),
           command->searchTerm().toLocal8Bit().constData(),
           command->filter().toLocal8Bit().constData(),
           command->offset(),
           command->limit());
    ...
    while (!done && !error) {
        ...
        qDebug("%s Calling mm_player_search with offset=%d, limit=%d", Q_FUNC_INFO, command->offset() + totalNodes, nodesThisChunk);
        m_mmPlayerCommandMutex.lock();
        ...
        m_mmPlayerCommandMutex.unlock();
        qDebug("%s mm_player_search: nodesThisChunk=%d, numRequested=%d, rc=%d", Q_FUNC_INFO, nodesThisChunk, numRequested, rc);
...
void CommandWorker::getTrackSessionItems(TrackSessionItemsCommand *command)
{
    ...
    if (rc == NO_ERROR) {
        ...
        // Free memory
        qDebug() << Q_FUNC_INFO << "Freeing node memory";
...
void CommandWorker::getCurrentTrackSessionInfo(TrackSessionInfoCommand *command)
{
    ...
    if (rc == NO_ERROR) {
        ...
        qDebug() << Q_FUNC_INFO << "Error getting current track session info";
