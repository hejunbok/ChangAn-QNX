...
#ifdef KBDAUTOCLOSER_DEBUG
        qDebug() << "MouseButtonPress event at (widget / app / screen):"
                 << mouseEvent->localPos() << mouseEvent->pos() << mouseEvent->globalPos();
#endif
        inputPoint = mouseEvent->pos();
    } else if (event->type() == QEvent::TouchBegin) {
        QTouchEvent *touchEvent = static_cast<QTouchEvent *>(event);
#ifdef KBDAUTOCLOSER_DEBUG
        QTouchEvent::TouchPoint tp = touchEvent->touchPoints().first();
        qDebug() << "TouchBegin event at (item / scene / screen):"
                 << tp.pos() << tp.scenePos() << tp.screenPos();
#endif
...
#ifdef KBDAUTOCLOSER_DEBUG
        qDebug() << "closing keyboard: input item rect is" << inputItemRect
                 << ", event point is" << inputPoint;
#endif
