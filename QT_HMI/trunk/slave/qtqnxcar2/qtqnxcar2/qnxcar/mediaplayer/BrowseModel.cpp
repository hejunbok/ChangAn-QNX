...
void BrowseModel::Private::browse(int mediaSourceId, QString mediaNodeId)
{
    ...
    if(mediaSource.ready) {
        ...
        // This is a synced source, and it's not ready. But, this means that there can still be a companion live view
        // which is ready, and so we should still fulfill the browse request by faking the contents of the root of the
        // synced source, as well as appending a virtual node to allow the user to start browsing the live view
        // immediately.
        qDebug("%s: Synced source is not ready, checking for ready companion live view...", Q_FUNC_INFO);
        ...
        if(liveMediaSource.id != -1 && liveMediaSource.ready) {
            // We found a companion live view that's ready.
            qDebug("%s: Found ready companion live view with ID: %d", Q_FUNC_INFO, liveMediaSource.id);
    ...
    if(lastBrowsedMediaNodeId() == QPlayer::ROOT_MEDIA_NODE_ID
        ...
        if(mediaSource.id != -1 && mediaSource.viewName == SOURCE_VIEW_NAME_SYNCED) {
            ...
            if(liveViewMediaSource.id != -1) {
                // We have a live source
                qDebug("%s: Found live view for device with UID: %s", Q_FUNC_INFO, qPrintable(mediaSource.uid));
