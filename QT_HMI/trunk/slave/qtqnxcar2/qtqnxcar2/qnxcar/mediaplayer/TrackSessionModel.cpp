...
void TrackSessionModel::Private::onTrackSessionChanged(QPlayer::TrackSessionEventType type, QPlayer::TrackSession trackSession)
{
    if(!m_trackSessionInitialized) {
        // Queue the event
        qDebug("%s: Queuing track session event of type %d", Q_FUNC_INFO, type);
        ...
        if(type == QPlayer::TRACK_SESSION_CREATED) {
            if(m_hasTrackSession && trackSession.id == m_trackSessionId) {
                ...
                qDebug("%s: Track session created. ID: %lld", Q_FUNC_INFO, trackSession.id);
            ...
            if(trackSession.id != m_trackSessionId) {
                qWarning("%s: Attempting to append to unknown track session ID. Current ID: %lld, Appended ID: %lld",
                         Q_FUNC_INFO,
                         m_trackSessionId,
                         trackSession.id);
            } else if(trackSession.length <= m_trackSessionLength) {
                qWarning("%s: Attempting to append with smaller or equal track session length. Current length: %d, Appended length: %d",
                         Q_FUNC_INFO,
                         m_trackSessionLength,
                         trackSession.length);
            } else {
                qDebug("%s: Track session appended. Current length: %d, Appended length: %d",
                       Q_FUNC_INFO,
                       m_trackSessionLength,
                       trackSession.length);
