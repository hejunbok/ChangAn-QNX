...
void SourcesModel::Private::onMediaSourceEvent(const QPlayer::MediaSourceEventType type, const QPlayer::MediaSource mediaSource)
{
    ...
    if(!m_sourcesInitialized) {
        ...
        qDebug("%s: Queuing media source event of type %d", Q_FUNC_INFO, type);
...
void SourcesModel::Private::processQueuedEvents()
{
    if(!m_queuedEvents.isEmpty()) {
        qDebug() << Q_FUNC_INFO << "Processing" << m_queuedEvents.length() << "queued media source event(s)";
...
void SourcesModel::Private::add(QnxCar::MediaPlayer::MediaSource mediaSource) {
    qDebug("%s\n" \
           ...
           mediaSource.id,
           mediaSource.type,
           qPrintable(mediaSource.name),
           qPrintable(mediaSource.viewName),
           mediaSource.ready,
           mediaSource.capabilities);
