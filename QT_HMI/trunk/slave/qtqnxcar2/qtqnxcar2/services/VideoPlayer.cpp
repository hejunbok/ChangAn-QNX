...
{
    if (hasContext()) {
        qWarning() << Q_FUNC_INFO << "Context already created for:" << contextName();
        return false;
    }

    qDebug() << Q_FUNC_INFO << "Trying to open context for:" << contextName();

    if (d->m_ppsControlObject == NULL){
        qDebug() << Q_FUNC_INFO << "Trying to create pps control object for:" << contextName();
        if (!d->createPPSControlObject()){
            qWarning() << Q_FUNC_INFO << "Cannot to create pps control object for:" << contextName();
            return false;
        }
    }
...
bool VideoPlayer::closeContext()
{
    ...
    qDebug() << Q_FUNC_INFO << "Closing context for:" << contextName();
...
void VideoPlayer::Private::detachVideoOutput()
{
    if(m_videoOutputId != -1) {
        qDebug() << Q_FUNC_INFO << "Detatching video output ID" << m_videoOutputId;
...
void VideoPlayer::Private::detachAudioOutput()
{
    if(m_audioOutputId != -1) {
        qDebug() << Q_FUNC_INFO << "Detatching audio output ID" << m_audioOutputId;
...
{
    qDebug() << "Context created for:" << q->contextName();
    Q_UNUSED(cmd);
    m_hasContext = true;
...
{
    qDebug() << "Context closed for:" << q->contextName();
    Q_UNUSED(cmd);
    m_hasContext = false;
...
{
    std::cout << "VideoPlayer::setViewPort(" <<  x << "," << y << "," << width << "," << height << ")" << std::endl;
...
{
    std::cout << "VideoPlayer::openUrl()" << std::endl;
...
{
    std::cout << "VideoPlayer::openUrlForce()" << std::endl;
...
{
    std::cout << "VideoPlayer::detach()" << std::endl;
    ...
    if(state() != IdleState) {
        ...
        qWarning() << Q_FUNC_INFO << "Attempting to detach input on already idle player";
    }
}
...
{
    std::cout << "VideoPlayer::playForce()" << std::endl;
...
{
    std::cout << "VideoPlayer::resume()" << std::endl;
