...
{
    qDebug() << "set clip position of window: " << window << point;
...
{
    qDebug() << "set position of window: " << window << point;
...
{
    qDebug() << "set clip size of window: " << window << size;
...
{
    qDebug() << "set clip size of source window: " << window << size;
...
{
    qDebug() << "set size of source window: " << window << size;
...
{
    qDebug() << "set size of window: " << window << size;
...
{
    qDebug() << "set sensitivity for" << window << "to" << mask;
...
{
    qDebug() << "set ZOrder for" << window << "to" << zvalue;
...
{
    qDebug() << "set visibility for" << window << "to" << visible;
...
void WindowManager::Private::processLocalRequestsForPid(Q_PID pid)
{
    ...
    qDebug() << "Setting properties for" << pid << "-" << data;
    Q_FOREACH(Window window, windowsByProcess.value(pid)) {
        ...
        if (winDataIt != data.windowSpecific.constEnd()) {
            winData = *winDataIt;
            qDebug() << "Window-specific properties for" << window.window << "-" << winData;
        }
        ...
        if (transparencyMode.isSet()) {
            ...
            qDebug() << "Setting transparency for" << pid << window.window << "to" << transparency;
            ...
            switch(type) {
                case SCREEN_EVENT_CREATE:
                {
                    ...
                    if (get_window_and_pid(screen_ev, &winHandle, &pid)) {
                        qDebug() << "SCREEN_EVENT_CREATE: pid: " << pid << " windowhandle: "  << winHandle;
                    }
                ...
                case SCREEN_EVENT_POST:
                {
                    ...
                    if (get_window_and_pid(screen_ev, &winHandle, &pid)) {
                        ...
                        qDebug() << "SCREEN_EVENT_POST: pid: " << pid << " windowhandle: " << winHandle << " windowgroup: " << windowGroup;
                        {
                ...
                case SCREEN_EVENT_CLOSE:
                {
                    ...
                    if (get_window_and_pid(screen_ev, &winHandle, &pid)) {
                        const QString windowGroup = get_window_group_name(winHandle);
                        qDebug() << "SCREEN_EVENT_CLOSE: pid: " << pid << " windowhandle: " << winHandle << " windowgroup: " << windowGroup;
                        QMutexLocker lock(&d->windowHandlingLock);
