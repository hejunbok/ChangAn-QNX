    ...
    , m_externalApplicationLaunching(true)
{
    ...
    if (m_ppsObject->isValid()) {
        ...
        std::cout << pps_kanziAppClickedKey << ":" << m_appClickedId << std::endl;
    } else {
...
void ApplicationManager::ppsAttributeChanged(const QString &name, const QPps::Variant &attribute)
{
    ...
    if (name == QString::fromLatin1(pps_kanziAppClickedKey))
    {
        ...
        std::cout << "applications().size():" << m_launcher->applicationListModel()->applications().size() << std::endl;
        for (int i = 0; i < m_launcher->applicationListModel()->applications().size(); i++)
        {
            std::cout << "applications[" << i << "].name:" << m_launcher->applicationListModel()->applications()[i].name().toLatin1().data() << std::endl;
        }
...
bool ApplicationManager::startApplicationId(const QString &appId)
{
    ...
    std::cout << "test......ApplicationManager::startApplicationId" << std::endl;
    std::cout << "test......appId=" << appId.data() << std::endl;
#ifdef CAR2_HOST_BUILD
    Q_UNUSED(appId);
    std::cout << "test......CAR2_HOST_BUILD" << std::endl;
    ...
    std::cout << "test......m_externalApplicationLaunching" << std::endl;
    ...
    std::cout << "test......m_launchingAppData.isValid()" << std::endl;
    ...
    std::cout << "test......m_launcher->setScreenGeometry(m_screenGeometry)" << std::endl;
    std::cout << "test......appData.name = " << appData.name().toLatin1().data() << std::endl;
    ...
    if (customIt != m_customApplications.constEnd()) {
        ...
        std::cout << "test......m_launcher->start(" << appId.data()  << ")"<< std::endl;
...
void ApplicationManager::onAppStarted(Q_PID pid) {
    qDebug("%s: Tracking launching application PID as %lld", Q_FUNC_INFO, pid);
...
void ApplicationManager::onAppStartFailed() {
    ...
    qDebug("%s: Clearing launching application information", Q_FUNC_INFO);
...
void ApplicationManager::onAppStopped(Q_PID pid) {
    ...
    if(pid == m_launchingAppPid) {
        qDebug("%s: Stopped application PID %lld matches launching application PID. Clearing launching application information", Q_FUNC_INFO, pid);
        m_launchingAppData = QnxCar::ApplicationData();
        m_launchingAppPid = 0;
    }
    ...
    if(pid == m_runningAppPid) {
        qDebug("%s: Stopped application PID %lld matches running application PID. Clearing running application information", Q_FUNC_INFO, pid);
...
void ApplicationManager::handleWindowPosted(Q_PID pid)
{
    ...
    if (m_launchingAppData.isValid()) {
        ...
        qDebug("%s: Window was posted for PID %lld, assumed for launching application with ID %s", Q_FUNC_INFO, pid, qPrintable(m_launchingAppData.id()));
        m_runningAppPid = pid;
    } else {
    ...
    qDebug("%s: Tracking the currently running application ID as %s", Q_FUNC_INFO, qPrintable(m_launchingAppData.id()));
