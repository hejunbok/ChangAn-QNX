...
Object::Object(const QString &path, bool delta, PublicationMode mode, bool create, QObject *parent)
    : QObject(parent)
    , d(new Private(this))
{
    d->path = path;

    const QString openPath = delta ? buildPath(path, QStringLiteral("delta")) : path;
    // the 3rd argument to open() is mandatory with O_CREAT, according to POSIX...
    //const int perm = S_IRUSR | S_IWUSR | S_IRGRP; // TODO configurable?
    std::cout << "open " << openPath.toLatin1().data() << std::endl;
    const int fd = open(openPath.toLocal8Bit(), openFlags(mode, create), 0666);
    if (fd == -1) {
        d->errorString = QObject::tr("Unable to open %1: %2").arg(d->path, QString::fromLocal8Bit(strerror(errno)));
        return;
    }
