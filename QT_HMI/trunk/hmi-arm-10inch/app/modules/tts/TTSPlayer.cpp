      ...
      void *userData)
{
   ...
   else
   {
      ...
      ret = ISSTTSGetAudioData(pThis->m_hTTS, pThis->m_PCMBuff, nSize / 2, &nGet, NULL, NULL);
...
TTSPlayer::TTSPlayer(const char* szResPath, const char* szMachineCode, ITTSPlayListener* pListener, QObject *parent)
    : QObject(parent),
   m_PCMBuff(NULL),
   m_hRes(NULL),
   m_hTTS(NULL),
