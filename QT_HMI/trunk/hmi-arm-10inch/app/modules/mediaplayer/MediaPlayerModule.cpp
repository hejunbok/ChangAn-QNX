    ...
    , m_video_FirstRun(true)
{
    ...
    if (m_ppsObject->isValid()) {
        ...
        //std::cout << pps_kanziVideoClickedKey << ":" << m_mediaClickedId << std::endl;
...
void MediaPlayerModule::ppsAttributeChanged(const QString &name, const QPps::Variant &attribute)
{
    ...
    if (name == QString::fromLatin1(pps_kanziMusicClickedKey))
    {
        ...
        if(1)
        {
            ...
            createTrackSession(0, QString::fromLatin1("/accounts/1000/shared/music"), m_musicClickedId);
        }
        else
        {
...
{
    std::cout << "MediaPlayerModule::createTrackSession   " << "mediaSourceId:" << mediaSourceId << "  mediaNodeId:" << mediaNodeId.toStdString() <<"  index:"<<index<< std::endl;
...
void MediaPlayerModule::playVideo(int mediaSourceId, QString mediaNodeId)
{
    ...
    std::cout << "MediaPlayerModule::playVideo   " << "mediaSourceId:" << mediaSourceId << "  tmpmediaNodeId:" << tmpNodeId.toUtf8().data()<< std::endl;
...
void MediaPlayerModule::onVideoUrlLoadedForce(QPlayer::ExtendedMetadataCommand *command)
{
    ...
    if(command->result().contains(urlKey)) {
        std::cout << "current video url : " << command->result().value(urlKey).toUrl().toString().toUtf8().data() << std::endl;
