...
#define AUDIO_SEC_INPUT_ADDR          0x28
...
int audio_init ( void )
{
  ...
  if ( !audio_rdy ) {
    ...
    /* Configure Host I2S output 0 */
    if ( audio_write ( AUDIO_PERIPHERAL_CFG_ADDR, host_output0_cfg, sizeof ( host_output0_cfg ) ) )
      goto err;
...
int audio_establish_physical_chnl ( void )
{
  ...
  for ( chnl = 0; chnl < AUDIO_PHYSICAL_CHNLS_CNT; chnl++ ) {
    ...
    if(chnl == AUDIO_SEC_PHYSICAL_CHNL) {
      MSG("Conect GPF.\n");
      MSG("AUDIO_X_GPF_LInPntr:%x--->%x\n",AUDIO_X_GPF_LInPntr,subaddr);
      audio_write_x_reg ( AUDIO_X_GPF_LInPntr, &subaddr, 1 );
      MSG("AUDIO_X_GPF_LInPntr:%x--->%x\n",AUDIO_X_GPF_RInPntr,subaddr);
      audio_write_x_reg ( AUDIO_X_GPF_RInPntr, &subaddr, 1 );
    }
  }
...
int audio_attach_input_to_chnl ( audio_input_name input, audio_physical_chnl_name chnl )
{
  ...
  case AUDIO_PRIM_REAR_PHYSICAL_CHNL:
    ...
    break;
  case AUDIO_SEC_PHYSICAL_CHNL:
    if ( AUDIO_NEA == physical_inputs[input].p_s_value ) {
      MSG ( "Invalid input for Secondary channel.\n" );
      return -EINVAL;
    }
    dsp_high_lvl_data = physical_inputs[input].p_s_value;
    err = audio_write ( AUDIO_SEC_INPUT_ADDR, &dsp_high_lvl_data, 1 );
    if ( err != 0 ) {
      DBG();
      return err;
    }
...
int audio_init_default_output ( void )
{
  ...
  if ( !err ) {
    err = audio_attach_output_to_chnl ( AUDIO_HOST_IIS_OUT0, AUDIO_SEC_PHYSICAL_CHNL );
  }
